...
func (fi *funcInfo) writeMock(out io.Writer) {
	...
	if fi.varidic {
		...
		if fi.IsMethod() {
			fmt.Fprintf(out, "_ctrl.Call(_emMock, \"%s\", args...)\n", fi.name)
		} else {
		...
		if fi.IsMethod() {
			fmt.Fprintf(out, "_ctrl.Call(_emMock, \"%s\"", fi.name)
		} else {
...
func (m *mockGen) pkg(out io.Writer, name string) error {
	...
	fmt.Fprintf(out, "\t_emMock interface{}\n")
	fmt.Fprintf(out, ")\n\n")
		...
		case *ast.GenDecl:
			...
			case token.IMPORT:
				if len(d.Specs) == 1 {
					...
					//if impPath == "github.com/onsi/ginkgo/ginkgo/testrunner" {
					//	fmt.Println(filename, ": Skipping github.com/onsi/ginkgo/ginkgo/testrunner...")
					//	continue
					//}
					//if impPath == "github.com/cloudfoundry/cli/cf/api/security_groups/defaults" {
					//	fmt.Println(filename, ": Skipping github.com/cloudfoundry/cli/cf/api/security_groups/defaults...")
					//	continue
					//}
					if s.Doc != nil {
